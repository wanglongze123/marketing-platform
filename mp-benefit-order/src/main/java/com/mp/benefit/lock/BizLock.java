...
public class BizLock {
    ...
    /**
     * 退款锁（含准入回收与退款执行）。键取主单号，<b>与支付回调、关单同键</b>。
     *
     * <p>同键是必需的：退款准入要读 {@code grant_status} 作判定，而查单收敛正在写它 —— 两者并发时
     * 准入可能读到一个正在变化的状态。正确性仍由条件更新兜底（L3），锁只是把并发压平（L2）。
     *
     * <p>V3 PR-7 引入。回收与退款执行共用一把：一单的逆向链路本就应当串行，分成两把锁会让 「回收还没写完，退款已经在读回收结果」成为可能。
     */
    public <T> T aroundRefund(String bizNo, Supplier<T> action) {
        return around(orderKey(bizNo), action);
    }
