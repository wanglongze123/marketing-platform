 ...
 * 裂变可靠任务访问。
 *
 * <p><b>与 {@code BenefitTaskMapper} 的 SQL 逐条同构，这是有意的</b>：任务表必须与业务状态同库 （技术方案 §1.3），四个库各有一张，因而各有一个
 * Mapper。抽公共父接口需要把表名参数化，而 MyBatis 注解式 SQL 里表名不能用 {@code #{}} 绑定（那是值占位符），只能字符串拼接 —— 用拼接
 * 换取「少写一份」，代价是把 SQL 注入面和可读性一起赔进去。
 *
 * <p>V4 拆服务后这两份会分属不同进程，届时「共用一份」反而成了跨服务耦合。
 *
 ...
 */
@Mapper
public interface FissionTaskMapper extends BaseMapper<FissionTask> {
     ...
     */
    @Update(
            "INSERT INTO fission_task (task_no, biz_no, task_type, op_no, status, next_time,"
     ...
     */
    @Select(
            "SELECT id FROM fission_task WHERE status = 'PENDING' AND next_time <= NOW(3)"
     ...
     */
    @Select(
            "SELECT id FROM fission_task WHERE status = 'DOING' AND lease_expire < NOW(3)"
    ...
    @Update({
        "<script>",
        "UPDATE fission_task SET status = 'DOING', lease_owner = #{owner},",
    ...
    @Select({
        "<script>",
        "SELECT * FROM fission_task",
        " WHERE id IN <foreach item='id' collection='ids' open='(' separator=',' close=')'>#{id}</foreach>",
        " ORDER BY id",
        "</script>"
    })
    List<FissionTask> selectByIds(@Param("ids") List<Long> ids);

    /** 完成。fencing：租约已易主则 {@code affected_rows = 0}，放弃写回。 */
    @Update(
            "UPDATE fission_task SET status = 'DONE', lease_owner = NULL, lease_expire = NULL"
     ...
     */
    @Update(
            "UPDATE fission_task SET status = 'PENDING', retry_count = retry_count + 1,"
     ...
     */
    @Update(
            "UPDATE fission_task SET status = 'PENDING',"
                    + " next_time = DATE_ADD(NOW(3), INTERVAL #{backoffMicros} MICROSECOND),"
                    + " lease_owner = NULL, lease_expire = NULL"
                    + " WHERE id = #{id} AND status = 'DOING' AND lease_owner = #{owner}")
    ...
    @Update(
            "UPDATE fission_task SET status = 'DEAD', retry_count = retry_count + 1,"
                    + " lease_owner = NULL, lease_expire = NULL"
                    + " WHERE id = #{id} AND status = 'DOING' AND lease_owner = #{owner}")
    ...
    @Update(
            "UPDATE fission_task SET lease_expire = DATE_ADD(NOW(3), INTERVAL #{leaseSeconds}"
     ...
     */
    @Update(
            "UPDATE fission_task SET payload = JSON_SET(COALESCE(payload, '{}'), '$.missStreak',"
     ...
     */
    @Select(
            "SELECT COUNT(*) FROM fission_task WHERE biz_no = #{bizNo}"
                    + " AND task_type = 'QUERY_GRANT' AND status IN ('PENDING', 'DOING')")
    int countOpenQueryGrant(@Param("bizNo") String bizNo);

    /** 可观测端点用：按业务号取全部任务快照。 */
    @Select("SELECT * FROM fission_task WHERE biz_no = #{bizNo} ORDER BY task_type, op_no")
     ...
     * lease_owner} 校验，两者不是同一类操作。
     *
     * <p>只对未完成的任务生效：已 {@code DONE} / {@code DEAD} 的不重置，避免把死信悄悄改成完成。
     */
    @Update(
            "UPDATE fission_task SET status = 'DONE', lease_owner = NULL, lease_expire = NULL"
                    + " WHERE biz_no = #{bizNo} AND task_type = #{taskType} AND op_no = #{opNo}"
                    + " AND status IN ('PENDING', 'DOING')")
    int markDoneByBizAndOp(
            @Param("bizNo") String bizNo,
            @Param("taskType") String taskType,
            @Param("opNo") String opNo);
}
